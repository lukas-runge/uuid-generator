...
export default async function generatetypeid(props: LaunchProps<{ arguments: Arguments.GenerateTypeID }>) {
    let { prefix, numberOfUUIDsToGenerate } = props.arguments;
    ...
    try {
      ...
      if (parseableNumber <= UUID_MAX_NUMBER) {
        const uuids = await generateUuids(() => typeid(prefix).toString(), parseableNumber, upperCaseLetters, UUIDType.TYPEID);
